 ...
 * objetivo de validar que las clases DTO se generan correctamente, tanto en
 * su contenido como en el proceso de creación (p. ej. para añadir un
 * ComentarioDTO a una PeliculaDTO). A estas pruebas unitarias se les ha
 * añadido tests de rendimiento para validar que su generación es rápida.
 * @author Grupo RMBJ
 * @version 3.0
 * @since 2.0
 */
...
public class DTOGeneracionTest {
	...
	public void testGenerateValoracionDTOCorrectly() {
		...
		assertTrue("Test".compareTo(
				testValoracionDTO.getTitulo())==0);
		assertFalse(testValoracionDTO.getValoracion()<9);
		ValoracionDTO expected = new ValoracionDTO(1,
				"AnotherTest", 7);
	...
	public void testGenerateComentarioDTOCorrectly() {
		...
		testComentarioDTO = new ComentarioDTO(testPeliculaDTO,
				testUsuarioDTO.getLogin(),
				"Es una gran pelicula de test!");
		assertEquals("Test", testComentarioDTO.getPelicula()
				.getTitulo());
		assertEquals("usuario", testComentarioDTO.getUsuario());
		assertFalse(testComentarioDTO.getContenido()
				.compareTo("Es una basurilla de test")==0);
		logger.info("Comprobando que el ComentarioDTO se ha creado"
				+ " en una diferencia de tiempo razonable...");
	...
	public void testSetComentarioDTOIncorrectly() {
		...
		testComentarioDTO = new ComentarioDTO(testPeliculaDTO,
				testUsuarioDTO.getLogin(),
				"Es una gran pelicula de test!");
		logger.info("Generando excepción al settear erróneamente"
				+ " el ComentarioDTO...");
	...
	public void testSetComentarioDTOCorrectly() {
		...
		testPeliculaDTO.setComentario(testComentarioDTO);
		logger.info("Ejecutando aserto al insertar un "
				+ "ComentarioDTO correctamente...");
		assertEquals("Es una gran pelicula de test!",
				testComentarioDTO.getPelicula().getComentarios()
						.get(0).getContenido());
		...
		logger.info("Ejecutando aserto al insertar la "
				+ "lista de ComentariosDTO...");
		assertEquals("Es una gran pelicula de test!",
				testComentarioDTO.getPelicula().getComentarios()
						.get(0).getContenido());
	}
