...
public class ServiceLocator {
	...
	public void registerUser(String login, String password) {
		...
		Response response = invocationBuilder.post(Entity.entity(usuarioDTO,
				MediaType.APPLICATION_JSON));
		if (response.getStatus() != Status.OK.getStatusCode()) {
			logger.warning("Error connecting with the server. Code: "
					+ response.getStatus());
		} else {
	...
	public UsuarioDTO getUser(String login, String pwd) {
		...
		sayHelloWebTarget = sayHelloWebTarget.queryParam(
				"login", login);
		Invocation.Builder invocationBuilder = sayHelloWebTarget.queryParam(
				"pwd", pwd).request(
				MediaType.APPLICATION_JSON);
	...
	public String sayMessage(String login, String password, String message) {
		...
		Invocation.Builder invocationBuilder = sayHelloWebTarget.request(
				MediaType.APPLICATION_JSON);
	...
	public void registerActor(String id, String nombre, String apellido,
		...
		WebTarget registerUserWebTarget = webTargetService.path(
				"registerActor");
		Invocation.Builder invocationBuilder = registerUserWebTarget.request(
				MediaType.APPLICATION_JSON);

		ActorDTO actorDTO = new ActorDTO(id, nombre, apellido, edad);
		Response response = invocationBuilder.post(Entity.entity(actorDTO,
				MediaType.APPLICATION_JSON));
		if (response.getStatus() != Status.OK.getStatusCode()) {
			logger.warning("Error connecting with the server. Code: "
					+ response.getStatus());
	 ...
	 */
	public void registerPelicula(String titulo, String sinopsis,
								 String genero, int duracion, int anyo,
								 String director, String enlacetrailer,
								 String premios, String seccion,
		...
		PeliculaDTO peliculaDTO = new PeliculaDTO(titulo, sinopsis, genero,
				duracion, anyo, director, enlacetrailer, 0,
				premios,null, seccion,
				strActores.toString(), imagen);

		logger.info("Enviando pelicula: " + peliculaDTO.getTitulo());
		Response response = invocationBuilder.post(Entity.entity(peliculaDTO,
				MediaType.APPLICATION_JSON));
		if (response.getStatus() != Status.OK.getStatusCode()) {
			logger.warning("Error connecting with the server. Code: "
					+ response.getStatus());
	...
	public void valorarPelicula(String titulo, float valoracion)
		...
		WebTarget registerValoracionWebTarget = webTargetService.path(
				"registerValoracion");
		Invocation.Builder invocationBuilder = registerValoracionWebTarget
				.request(MediaType.APPLICATION_JSON);
	...
	public ArrayList<String> getFiltroList(String filtro)
		...
		if (peliculasFestival.size() == 0) {
			...
			if (filtro.compareTo("Género")==0) { // Filtro para Género.
			...
			else if (filtro.compareTo("Sección del Festival")==0) {
				...
				Map<String, String> filtersMap = new TreeMap<String, String>();
				for (PeliculaDTO aux : peliculasFestival) {
					if (!(filtersMap.containsKey(Integer.toString(
							aux.getAnyo())))) {
						filtersMap.put(Integer.toString(aux.getAnyo()),
								Integer.toString(aux.getAnyo()));
					}
				}
				filtrosPelis.addAll(filtersMap.values());
				return filtrosPelis;
